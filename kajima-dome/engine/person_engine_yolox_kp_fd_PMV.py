...
def draw_box(image, box, label='', color=BGRColor.CYAN, font_size=0.7):
    ...
    # logging.info("FONT SIZE {}", textsize)
...
def draw_box_batch(image, bboxs, tids, face_labels, pmvs):
    # label = "tid_"+str(tids)+"_"+face_labels 
    # logging.info("LABELS ", face_labels)
...
class PersonEngine(object):
    def __init__(self, cfg_person, cfg_cam, face_details, body_details, redis_conn) -> None:
        ...
        logging.info('Loaded {}'.format(cfg_person))
        logging.info('{}'.format(cfg_cam))
        ...
        logging.info('Detection Engine loaded ...')
        ...
        logging.info('Skeleton Engine loaded ...')
    ...
    def YoloxPersonBoxTrackMatchFeatureBatch(self, bgrimgs):
        ...
        t2 = time.time() - t1
        logging.info("Detections time : {}".format(t2))
        ...
        if org_dts[0] is not None and org_dts[1] is not None:
            ...
            logging.info("Face Confs {}".format(face_confs))
            ...
            logging.info("TRACK ids label : {} {}".format(trackids, subids))
            ...
            for i in range(len(bgrimgs)):          
                ...
                cv2.putText(bgrimgs[i],"Detection count = {}".format(len(roi_dts[i])),(10,100),cv2.FONT_HERSHEY_SIMPLEX,3,(0,255,0),2,cv2.LINE_AA)
                # logging.info("Detection count = {}".format(len(roi_dts[i])))
            ...
            for i in range(len(bgrimgs)):          
                cv2.polylines(bgrimgs[i], [self.cam_roi[i]], True, color=BGRColor.CYAN, thickness=2)
                cv2.putText(bgrimgs[i],"Detection count = 0",(10,100),cv2.FONT_HERSHEY_SIMPLEX,3,(0,255,0),2,cv2.LINE_AA)
            logging.info("Detection count = 0")
            self.FaceBodyTracker.Increment_track_age()
            return None

        # logging.info("Face Conf ", face_conf0, face_conf1)
...
class DetectionEngine(object):
    def __init__(self, pd_model, pd_det_threshold=0.5, pd_nms_threshold=0.3, pd_input_resize=0, 
        ...
        logging.info("{} {} {} {} {} {} {}".format(pd_model, pd_det_threshold, pd_nms_threshold, pd_input_resize, max_detected_persons, min_person_width, device))
    ...
    def YoloxPersonDetect(self, bgrimg):
        ...
        t2 = time.time() - t1
        logging.info("YoloX Detection time : {}".format(t2))
    ...
    def YoloxPersonDetectBatch(self, bgrimgs):
        ...
        t2 = time.time() - t1
        logging.info("YoloX Detection time batch {}".format(t2))
...
class SkeletonEngine(object):
    ...
    def SkeletonDetect(self, bgrimg, dts):
        ...
        #crop upper body
        t2 = time.time() - t1
        logging.info("Skeleton Detection time {}".format(t2))
    ...
    def SkeletonDetectBatch(self, bgrimgs, detections):
        ...
        t2 = time.time() - t1
        logging.info("Skeleton Detection time batch {}".format(t2))
...
class FeatureEngine(object):
    ...
    def __get_scale(self, img, target_size):
        ...
        # logging.info("img shape: {}, min: {}, max: {}".format(img.shape, im_size_min, im_size_max))
    ...
    def __crop_rot_upper_body_lmk(self, input_img, detection, landmark, lmk_conf):
        ...
        logging.info(colored("CROPPING WITH LMK", "blue"))
    ...
    def FeatureExtractionBatch(self, bgrimgs, detections, lmks, lmk_confs):
        ...
        t2 = time.time() - t1
        logging.info("Feature ectraction time: {}".format(t2))
    ...
    def __action_recognition(self, lmks, img_size):
        ...
        if len(polar_lmks)>0:
            with torch.no_grad():
                ...
                logging.info("action {}".format(actions))
    ...
    def FeatureExtractionBatch_org(self, bgrimgs, detections, lmks, lmk_confs):
        ...
        for i, dts in enumerate(detections):
            ...
            logging.info("FD size: {}".format(fd_face_crop.shape))
            detector_output = self.__detect_faces(fd_face_crop, max_faces=1)
            if detector_output is None:
                chips = []
                chip = fd_face_crop[16:128, 16:128, :]
