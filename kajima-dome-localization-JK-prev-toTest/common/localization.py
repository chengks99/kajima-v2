...
def flatten(points):  
    for point in points :
        if isinstance(point,Iterable) and not isinstance(point,(str,bytes)) :
            yield from flatten(point)
        else :
            yield point
...
class Localize(object) :
    ...
    def find_location(self,info) :
        ...
        self.get_point(info)
        # print(self.opf_standing) ## Output per framne
        # print(self.opf_sitting) 
        ...
        return self.combined_results, now, dt_string

    def floorplan_location_standing(self,point) : 
        point = list(flatten(point))
        # print(point)
        # print(self.camera.hom_mat_stand)
    ...
    def undistort_points(self,point) :
        ...
        # logging.info(point)
        # logging.info(cam_int)
        # logging.info(self.camera.ud_vector)

        cur_points = cv2.undistortPoints(np.asarray(point),cam_int,self.camera.ud_vector,cam_int)
