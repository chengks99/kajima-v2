            ...
            try:
                ...
                if _output:
                    ...
                    if boxes is None:
                        ...
                        clothids = [v for i, v in enumerate(clothids) if i not in index2delete]
                        actions = [v for i, v in enumerate(actions) if i not in index2delete]
                        if len(boxes) > 0 :
                            ...
                            ## Print PMV, actions, human_ID and clothatt
                            for pmv,act,hid,clothid in zip(pmvs,actions,face_labels,clothids) :
                                if act == 0 :
                                    pose = "sit"
                                else :
                                    pose = "stand"
                                logging.info("Results : pmv_num : {:.2f}, pose : {}, human_id : {}".format(pmv,pose,hid))
                                logging.info("Results : Clothing = Top : {} conf : {:.2f}".format(clothid["top"][0],clothid["top"][1]))
                                logging.info("Results : Clothing = Bottom : {} conf : {:.2f}".format(clothid["bot"][0],clothid["bot"][1]))
                                logging.info("Results : Clothing = Full : {} conf : {:.2f}".format(clothid["full"][0],clothid["full"][1]))
