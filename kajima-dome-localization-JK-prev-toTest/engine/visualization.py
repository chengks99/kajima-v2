...
def draw_yolox(img, output, color=(255,0,0), conf=0.5):
    ...
    for i in range(len(output)):
        ...
        # cv2.putText(img, str(theta), (x0, y0 - 10), fontFace=0, fontScale=1.0, color=color, thickness=3)
        ...
        # cv2.polylines(img, [contours], isClosed=True, color=color,
        #         thickness=line_width, lineType=cv2.LINE_4)
...
def draw_lmks(img, landmarks, landmark_confs, color, LMK_VISIBILITY_THRESHOLD):
    ...
    # without limbs
    line_pairs = [
        [0,1],[0,2],[1,3],[2,4],[0,17],[17,5],[17,6],[17,18],[18,11],[18,12]
    ]
