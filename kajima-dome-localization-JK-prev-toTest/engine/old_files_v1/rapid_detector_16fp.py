...
sys.path.append(str(scriptpath.parent / 'rapid'))
from models.rapid_export import RAPiD
from utils.utils import rect_to_square, nms, detection2original
...
class RapidDetector():
    ...
    def __init__(self, model_name='', weights_path=None, device=-1, 
        ...
        #     else:
        #         print("TORCH GPU initialization error")
        ...
        logging.debug('Successfully initialized model {}'.format(model_name))
        # total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        # print(f'Successfully initialized model {model_name}.',
        #     'Total number of trainable parameters:', total_params)
        
        self.rapid_model.load_state_dict(torch.load(weights_path)['model'])
        logging.debug('Successfully loaded weights: {}'.format(weights_path))

        # self.rapid_model = torch.nn.DataParallel(self.rapid_model, device_ids=[self.gpuid]).cuda()
        self.rapid_model.eval()

        self.rapid_model = self.rapid_model.to(self.device)
        
        self.rapid_model.half() #JK
    ...
    def predict_pil(self, pil_img):
        ...
        input_img, _, pad_info = rect_to_square(pil_img, None, input_size, 0)
        ...
        with torch.no_grad():
            dts = self.rapid_model(input_.half()).cpu()
        ...
        dts = nms(dts, is_degree=True, nms_thres=self.nms_thresh, img_size=input_size)
        dts = detection2original(dts, pad_info.squeeze())
    ...
    def predict_pil_batch(self, pil_imgs):
        ...
        batch_img = []
        for img in pil_imgs:
            input_img, _, pad_info = rect_to_square(img, None, input_size, 0)
            input_ori = tvf.to_tensor(input_img)
            input_ = input_ori.unsqueeze(0)
        ...
        with torch.no_grad():
            dts = self.rapid_model(batch_img.half()).cpu()
        ...
        for i in range(len(dts)):
            ...
            detection = nms(detection, is_degree=True, nms_thres=self.nms_thresh, img_size=input_size)
            detection = detection2original(detection, pad_info.squeeze())
