...
from person_tracker_PMV_KF import FishEyeTracker
...
class PersonEngine(object):
    def __init__(self, pd_model, pr_model, kp_model, kp_cfg, 
        ...
        self.actionNet.load_state_dict(torch.load(model_location, map_location="cpu"))
    ...
    def YoloxPersonBoxTrackMatchFeature_no_thread(self, bgrimg):
        ...
        kf_det = self.detection_engine.YoloxPersonDetect(bgrimg)
        self.frame_num += 1

        # if dts is None:
        #     kf_det = self.kf_tracker.update()
        # else:
        ...
        #     kf_det = self.kf_tracker.update(dts)
        #     print("org persons ", len(dts))

        if kf_det is None:
            return None 

        #skeleton detection
        lmks, lmk_confs, new_dts = self.skeleton_engine.SkeletonDetect(bgrimg, kf_det)
...
class DetectionEngine(object):
    ...
    def __filterDts(self, dts):
        ...
        area = dts[:,2]*dts[:,3]
        dts = dts[area>self.min_person_area]
        if len(dts):
...
class SkeletonEngine(object):
    ...
    def __get_angle_yolox_batch(self, imgw, imgh, detections):
        dir = []
        for bb in detections:
            x,y,w,h = bb[:4]
            theta = self.__get_angle(imgw, imgh, x, y)
    ...
    def __get_angle_yolox(self, imgw, imgh, x, y):
        center_x, center_y = imgw/2, imgh/2
        A = math.atan((center_y-y)/(center_x-x))
    ...
    def SkeletonDetect(self, bgrimg, dts):
        ...
        #get box angles
        # rot = self.__get_angle_yolox_batch(imgw, imgh, dts)
        ...
        # # #create cxcywh, based on skeleton points
        # select_lmk = [True, True, True, True, True, True, True, False, False, False, False, True, True, False, False, False, False]
        # top_lmks = lmks[:, select_lmk]
        # xs = top_lmks[:,0]
        # xmin = np.minimum(top_lmks, 0)
        # ymin = np.minimum(top_lmks, 1)
        # xmax = np.maximum(top_lmks, 0)
        # ymax = np.maximum(top_lmks, 1)

        # # # new_dts =np.minimum(top_lmks[:,0], top_lmks[:,1])
        # print(top_lmks)
        # print("xmin ", xmin)
        # # # print(new_dts)
