...
class FishEyeTracker:
    def __init__(self, pr_threshold, body_db_file,
        ...
        self.tr_threshold = 0.3 #0.5 #pr_threshold #
        ...
        self.columns = [
            ...
            'iclo', 'iclo_kalman', 'pmv', 'pmv_kalman', 'action', 'kid'
        ]
    ...
    def __update_track(self, trackid, bbox, lmk, lmk_conf, feature, ftscore, iou, clothAtt, action):
        """
        ...
        if iou<0.5:
            self.tracks.at[idx, 'bbox'] = bbox[:4]
            self.tracks.at[idx, 'age'] = 0
            self.tracks.at[idx, 'kid'] = int(bbox[4])

        feat_sim = -1
        if self.tracks.at[idx, 'flag']==True:
            feat_sim = ftscore 
        else:
            feat_sim = 1

        #update feature when overlap with other boxes is below threshold and feature similarity is above threshold
        if feat_sim>self.pr_threshold:
        ...
        curr_cloth_att = self.tracks.at[idx, 'cloth_att']
        smooth_iclo = icloKalmanFilter.update(curr_cloth_att['iclo'])
    ...
    def __delete_old_tracks(self):
        ...
        print("TRACKS TO DELETE ", idx_to_del)
        if len(idx_to_del)>0:
            self.tracks = self.tracks.drop(index=idx_to_del)
        logging.debug("Deleted old track")
...
#        rows_to_match = self.tracks.loc[(self.tracks['age'] == 0)]
    ...
    def update(self, rects, lmks, lmk_confs, features, img_dim, iclo, actions):
        ...
        if len(self.tracks)==0:
            ...
            matching_score = -1
            for i in range(len(features)):
                lmkconf = np.array(lmk_confs[i])
                lmkconf = np.sum(lmkconf[:7])/7.0
                norm = np.linalg.norm(features[i])
                if norm>self.norm_threshold and lmkconf>0.3:
        ...
        # otherwise, try to match the input features to existing object
            ...
            sub_id = np.stack(self.tracks['sub_id'].tolist())
            kf_id = np.stack(self.tracks['kid'].tolist())
            dblmks = np.stack(self.tracks['lmk'].tolist())
            ...
            lmk_index, lmk_scores = MatchLmks(lmks, dblmks, dist_thresh=50.0)
...
#            non_matched_index = []     
            matching_score = last_scores
            for i in range(len(rects)):
                kfid = int(rects[i,4])
                tid = -1
                for j in range(len(kf_id)):
                    if kfid==kf_id[j]:
                        tid = j
                ...
                # if last_index[i]!=-1 and tid!=-1 and track_id[last_index[i]]==track_id[tid]:
                if last_index[i]!=-1 and last_index[i]==tid and lmk_index[i]==tid:
                    pmv = self.__update_track(track_id[last_index[i]], bboxs[i], lmks[i], lmk_confs[i], features[i], last_scores[i], iou[i], iclo[i], actions[i])
                    # track_index[i] = track_id[ct_index[i]]
                    subids[i] = sub_id[last_index[i]]
                    trackids[i] = track_id[last_index[i]]
                    pmvs[i] = pmv #person_details[ct_index[i]]
                #KF_id already eixists
                # elif kfid in self.tracks['kid'].values:  # Exists
                elif lmk_index[i]!=-1 and lmk_index[i]==tid:
                    pmv = self.__update_track(track_id[tid], bboxs[i], lmks[i], lmk_confs[i], features[i], last_scores[i], iou[i], iclo[i], actions[i])
                    # track_index[i] = track_id[ct_index[i]]
                    subids[i] = sub_id[tid]
                    trackids[i] = track_id[tid]
                    pmvs[i] = pmv #person_details[ct_index[i]]
                elif last_index[i]!=-1 and last_index[i]==tid:
                    pmv = self.__update_track(track_id[tid], bboxs[i], lmks[i], lmk_confs[i], features[i], last_scores[i], iou[i], iclo[i], actions[i])
                    # track_index[i] = track_id[ct_index[i]]
                    subids[i] = sub_id[tid]
                    trackids[i] = track_id[tid]
                    pmvs[i] = pmv #person_details[ct_index[i]]
                elif last_index[i] != -1 and last_index[i]==lmk_index[i]:   
                    pmv = self.__update_track(track_id[last_index[i]], bboxs[i], lmks[i], lmk_confs[i], features[i], last_scores[i], iou[i], iclo[i], actions[i])
                    # track_index[i] = track_id[ct_index[i]]
                    subids[i] = sub_id[last_index[i]]
                    trackids[i] = track_id[last_index[i]]
                ...
                elif dist[i] < self.dist_threshold and tid != -1:
                    pmv = self.__update_track(track_id[tid], bboxs[i], lmks[i], lmk_confs[i], features[i], last_scores[i], iou[i], iclo[i], actions[i])
                    # track_index[i] = track_id[ct_index[i]]
                    subids[i] = sub_id[tid]
                    trackids[i] = track_id[tid]
...
#                    non_matched_features.append(features[i])
                    ...
                    if kfid in self.tracks['kid'].values:  # Exists
                        print(colored("Track not added, kdif already exists", 'red'))
                        continue
...
def MatchLmks(ilmks, dblmks, dist_thresh):
    """
    Do an N:M comparison of feature vectors.
    Pass a single feature vector to <features> to do 1:M comparison.
    Pass a single feature vector to <feat_to_compare_with> to do N:1 comparison.
    ...
    lmk_curr = np.array(ilmks)
    lmk_prev = np.array(dblmks)

    num_curr = lmk_curr.shape[0]  # "current frame" features
    num_prev = lmk_prev.shape[0]  # "prev frame" or "database" features

    dist_mat = np.zeros((num_prev, num_curr), dtype="float32")
    for i in range(num_prev):
        lmk1 = lmk_prev[i]
        lmk1 = lmk1[:7,:] #extract first 7 points only
        for j in range(num_curr):
            lmk2 = lmk_curr[j]
            lmk2 = lmk2[:7,:] #extract first 7 points only
            xdis = (lmk1[:,0]-lmk2[:,0])**2
            ydis = (lmk1[:,1]-lmk2[:,1])**2

            dist_mat[i,j] = np.sqrt(np.sum(xdis+ydis))/7.0 #normalize with 7-points
    ...
    # Return matrix with num_curr elements, all set to -1:
    return_indices = np.full(num_curr, -1)
    dis_scores = np.full(num_curr, -1.0)

    for i in range(num_curr):
        # Find index of the highest sim score (row: previous, col: current):
        # This means the highest similarity value will be taken first.
        best_idx = np.unravel_index(np.argmin(dist_mat), shape=dist_mat.shape)

        if debug:
            # print("i:", i)
            # # print("sim:\n{}".format(sim))
            # # print("best_idx:", best_idx)
            # print("dist_mat[best_idx].item()", dist_mat[best_idx].item())
            logging.debug("MatchLMKs dist_mat[best_idx].item(): {}".format(dist_mat[best_idx].item()))

        if dist_mat[best_idx] > dist_thresh:
            # This means there are no more similarity scores above the threshold (default=0.3)
            # By breaking, the remaining return_indices will be left as -1.
            # print(">> break\n")
            break
            ...
            # from feat_existing:
            return_indices[best_idx[1]] = best_idx[0]
            dis_scores[best_idx[1]] = dist_mat[best_idx]

            # Set the whole column to max_value. "This face has been checked already."
            dist_mat[:, best_idx[1]] = max_dist

            # Set the entire row to max_valye for next iteration, to prevent multiple columns
            # being matched to one row (e.g. "two faces to the same name"):
            dist_mat[best_idx[0], :] = max_dist

            if debug:
                # print("return_indices: {}".format(return_indices))
                # print()
                logging.debug("MatchLMKs return_indices: {}".format(return_indices))
    ...
    # print(return_indices, sim_scores)
    return return_indices, dis_scores
