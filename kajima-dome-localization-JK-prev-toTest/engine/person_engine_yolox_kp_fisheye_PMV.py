...
class PersonEngine(object):
    ...
    def YoloxPersonBoxTrackMatchFeature(self, bgrimg, new_dts, lmks, lmk_confs, body_crops, body_features):
        ...
        if new_dts is None or bgrimg is None:
            trackids, subids, pmvs, score, clothids = self.BodyTracker.update(None, None, None, None, None, None, None)
    ...
    def worker(self, input_q, output_q):
        ...
        while True:
            ...
            # # else:
            # #     output_q.put(self.YoloxPersonBoxTrackMatchFeature(None, None, None, None, None, None))
...
class DetectionEngine(object):
    ...
    def worker(self, input_q, output_q):
        while True:
            ...
            #     output_q.put((self.YoloxPersonDetect(bgrimg)))
            # else:
...
class SkeletonEngine(object):
    ...
    def SkeletonDetect(self, bgrimg, dts):
        ...
        return bgrimg, dts, lmks, lmk_confs
    ...
    def worker(self, input_q, output_q):
        while True:
            ...
            #     output_q.put(self.SkeletonDetect(bgrimg, dts))
            # else:
            #     output_q.put((None, None, None, None))
            ...
            #     output = input_q.get()
            #     bgrimg, dts = output
            #     # print("Skeleton worker")
            #     # img, lmks, lmk_confs, new_dts = self.SkeletonDetect(bgrimg, dts)
            #     # output_q.put((img, new_dts, lmks, lmk_confs))
            #     output_q.put((self.SkeletonDetect(bgrimg, dts)))
            #     # input_q.task_done()
...
class FeatureEngine(object):
    ...
    def BodyFeatureExtraction(self, bgrimg, dts, lmks, lmk_confs):
        ...
        return bgrimg, dts, lmks, lmk_confs, body_crops, np.array(body_features)
    ...
    def worker(self, input_q, output_q):
        while True:
            ...
            #     output_q.put((self.BodyFeatureExtraction(bgrimg, dts, lmks, lmk_confs)))
            # else:
            #     output_q.put((None, None, None, None, None, None))

            # # time.sleep(0.1)

            output = input_q.get()
            bgrimg, dts, lmks, lmk_confs = output
            output_q.put((self.BodyFeatureExtraction(bgrimg, dts, lmks, lmk_confs)))
