...
import cv2
from camera2_offline import CamHolder, FloorHolder, MicHolder, DetectionEngine, CameraStream
import pandas as pd
...
class Camera(Adaptor):
    def __init__(self, args, **kw):
        ...
        self.df = pd.DataFrame(columns=['frame_num','human_id','X','Y','pmv','top','top_acc','bot','bot_acc','full','full_acc'])
    ...
    def run_init (self):
        ...
        if body_details is None:
            ...
            print('Body Feature Length: {}'.format(len(body_details.get('fvList', []))))
    ...
    def process_stream (self):
        # src = "7003_test_fps5.mp4"
        src = glob.glob("/home/ewaic/Kajima_Fisheye/camera_test2/7114_frame/*.png")
        # src = glob.glob('/home/jk/Desktop/GEARCamRecorded/frames_7114_test.mp4_partial/*.png')
        # src = glob.glob('/home/jk/Desktop/Accuracy result/Original frame/7114_frame/*.png')
        # src = glob.glob('/home/jk/Desktop/1/*.png')
        count = 0
        self.stream = []
        for image in sorted(src) :
            # print(count) 
            cur_image = cv2.imread(image)
            self.stream.append(cur_image)
            count += 1
        # self.stream.start()
        ...
        # while True:
        #     _success, _img = videocapture.read()
        #     if _success:
        #         self.stream.append(_img)
        #         count += 1
        #     else:
        #         break
        print("IMAGES IN STREAM ", count)
    ...
    # vid_writer = cv2.VideoWriter(save_path, cv2.VideoWriter_fourcc(*"mp4v"), fps, (int(width), int(height)))
        ...
        for image in self.stream :
            ...
            _img = image
            # logging.info("XXXXXXXXXXXXXXXXXXXXXXXXXXX")
            ...
            if _output :
                boxes, person_crop, face_labels, tids, lmks, image, pmvs, actions,score, clothatt  = _output
            else :
                boxes = None
            ...
            if self.view:
                ...
                cv2.imwrite("/home/ewaic/Code_to_test/Kajima_Fisheye_JK_prev/camera_test/7114_new/{}.png".format(str(frame_num_count).zfill(5)),image)
            ...
            if boxes is not None:
            # logging.info(_output)
            ## Check the results will be box after the editing 
                cur_res = _res["result"]
                logging.info("RESULTS")
                logging.info("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXxx")
                logging.info(cur_res)
                logging.info(pmvs)
                logging.info(clothatt)

                for res in cur_res : 
                    count_ = 0
                    for people,cloth in zip(res["list"],clothatt) :
                        logging.info(people)
                        logging.info(cloth)
                        current_res_X = people[0]
                        current_res_Y = people[1]
                        humanID = people[2]


                        to_be_added = {'frame_num' : str(frame_num_count).zfill(5),
                        'human_id' : humanID,
                        'X' : current_res_X,
                        'Y' : current_res_Y,
                        'pmv' : pmvs[count_],
                        ...
                        'bot_acc' : cloth["bot"][1],
                        'full': cloth["full"][0],
                        'full_acc': cloth["full"][1],
                        }
                        count_ += 1
                        logging.info(to_be_added)
                        logging.info("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXxx")
                        self.df = self.df.append(to_be_added, ignore_index = True)
                frame_num_count += 1
                # self.df = pd.DataFrame(columns=['frame_num','human_id','X','Y','pmv','top','bot','full'])
                # self.utility_list.append(len(resList[0]['list']) if resList is not None else 0)
                self.df.to_csv("7114_results.csv",index=False)
