...
def draw_box(image, box, writing, label='', color=BGRColor.CYAN, font_size=0.7):
    """
    ...
    cv2.rectangle(img, (bbox[0], bbox[1]-5-textsize[1]), (bbox[0]+textsize[0], bbox[1]-5), color=writing, thickness=-1)
...
class Camera(Adaptor):
    def __init__(self, args, **kw):
        ...
        ## ROI -- camera localization rate
        self.ROI_localization = np.array(self.cfg.get('cam_roi_2', [0,0,0,2991,2991,2991,2991,0]), dtype=np.int)
        ROI_length_localization = len(self.ROI_localization) // 2
        self.ROI_localization = self.ROI_localization.reshape(ROI_length_localization,-1)
            ...
            try:
                ...
                if _output:
                    ...
                    if boxes is None:
                        ...
                        logging.debug("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
                    else :
                        ## HERE IS THE LOCALIZATION BOXES SEPARATION -- BBOX localization is much bigger in size so need to remove it first
                        index2delete = []
                        # logging.debug("ALL BOXES ARE : {}".format(str(boxes)))
                        for i in range(len(boxes)) :
                            removed = remove_outside_bbox(self.ROI_localization ,boxes[i])
                ...
                if len(tids) > 0 :
                    ## HERE IS THE UTILITY BOXES SEPARATION -- BBOX localization is much bigger in size so need to remove it first
                    index2delete = []
                    # logging.debug("ALL BOXES ARE : {}".format(str(boxes)))
                    for i in range(len(boxes)) :
                        ...
                        if removed == -1 :
                            ...
                            continue
                    index2delete = set(index2delete)
                    boxes_utility = [v for i, v in enumerate(boxes) if i not in index2delete]

                if self.view:
                    cv2.namedWindow('Video feed', cv2.WINDOW_NORMAL)
                    cv2.resizeWindow("Video feed", 720, 720)
                    ...
                    for i in range(len(tids)):
                        ...
                        if i in index2delete :
                            color_writing = BGRColor.GRAY
                        else :
                            color_writing = BGRColor.BLACK
                        image = draw_box(image, boxes[i], color_writing, label, color=box_color, font_size=2.0)
                    ...
                    cv2.polylines(image, [self.ROI_localization], True, color=BGRColor.LIME, thickness=10)
                ...
                self.utility_list.append(len(boxes_utility) if boxes_utility is not None else 0)
